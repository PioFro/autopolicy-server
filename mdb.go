...
func (db *MongoDB) AddProfile(profile Profile, id Identity) (Profile, error) {
	...
	err = db.profiles.FindOne(bson.M{"apid": bson.ObjectIdHex(mport.APID)}).Exec(mprofile)
	if err != nil {
		...
		dbg(1, "Add Profile", "Profile for that APID is already posted.")
